...
sim1$Is_Active <- factor(sim1$Is_Active, levels = c("active", "inactive"), labels = c("True associations", "False associations"))
...
ggplot() + 
  ...
  geom_point(data = sim1_sensitivity %>% filter(Is_Active == "True associations"), 
             aes(x = as.numeric(Splicing_Num), y = 50 * (Sensitivity - 0.2)), size = 0.6, alpha = 0.8, colour = "#7570b3") +
  geom_line(data = sim1_sensitivity %>% filter(Is_Active == "True associations"), 
            aes(x = as.numeric(Splicing_Num), y = 50 * (Sensitivity - 0.2)), size = 0.6, alpha = 0.8, colour = "#7570b3") +
  geom_abline(intercept = 3.0, slope = 0, colour = "#d73027", alpha = 0.6, linetype = "longdash") + 
  my_theme() +
  labs(x = "Number of possible associations", y = "Log(Bayes factor)", fill = "") +
  theme(legend.position = "bottom") + 
  scale_fill_manual(values = c("True associations" = "#b2182b", "False associations" = "#bababa")) +
...
sim2$Is_Active <- factor(sim2$Is_Active, levels = c("active", "inactive"), labels = c("True associations", "False associations"))
...
ggplot() + 
  ...
  geom_point(data = sim2_sensitivity %>% filter(Is_Active == "True associations"),
             aes(x = as.numeric(Splicing_Num), y = 50 * (Sensitivity - 0.2)), size = 0.6, alpha = 0.8, colour = "#7570b3") +
  geom_line(data = sim2_sensitivity %>% filter(Is_Active == "True associations"),
            aes(x = as.numeric(Splicing_Num), y = 50 * (Sensitivity - 0.2)), size = 0.6, alpha = 0.8, colour = "#7570b3") +
  geom_abline(intercept = 3.0, slope = 0, colour = "#d73027", alpha = 0.6, linetype = "longdash") +
  my_theme() +
  labs(x = "Number of associated variants", y = "Log(Bayes factor)", fill = "") +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = c("True associations" = "#b2182b", "False associations" = "#bababa")) +
  scale_y_continuous(limits = c(-10, 40), breaks = c(-10, 0, 3, 10, 20, 30, 40), sec.axis = sec_axis(~ . * (1 / 50) + 0.2, name = "Sensitivity")) +
