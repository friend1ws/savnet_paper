...
source("gg_seq_logo.R")

splicing_mutation <- read.table("../output/omega.splicing_mutation.info.txt", sep = "\t", header = TRUE, quote = "", stringsAsFactors = FALSE) %>%
...
for(pos in c(2, 3, 7, 8, 9)) {
  ...
  motif_sub <- splicing_mutation %>% 
    ...
    filter(GSM2 == "No change") %>% filter(FKPM >= 10.0)
  
  tgg_logo_no <- gg_seq_logo(t(get_base_count_mat(motif_sub$Motif_Seq, 9)), TRUE) + 
    labs(x = "", y = "") +
    theme_nothing() 
  ...
  tgg_logo_yes <- gg_seq_logo(t(get_base_count_mat(motif_sub$Motif_Seq, 9)), TRUE) + 
    labs(x = "", y = "") +
    theme_nothing() 
...
plot_grid(ggdraw() + draw_label("Mutation position at splicing donor sites", angle = 90),
          plot_grid(
            ...
            ggdraw() + draw_label("Intronic position"), 
            ncol = 1, rel_heights = c(5.7, 0.25)),
          ncol = 2, rel_widths = c(0.05, 1))


ggsave("../output/seqlogo_list_simple.pdf", width = 6, height = 6)
...
seq_logo_print_list <- list(plot_grid(ggdraw() + draw_label(""),
                                      ggdraw() + draw_label("No change"),
                                      ggdraw() + draw_label("ES"),
                                      ggdraw() + draw_label("A5S"),
                                      ggdraw() + draw_label("IR"),
...
for(pos in c(2, 3, 7, 8, 9)) {
  ...
  if (count_mat[pos, 2] >= count_thres) {
    tgg_logo_ES <- gg_seq_logo(t(get_base_count_mat(motif_sub$Motif_Seq, 9)), TRUE) + 
      labs(x = "", y = "") +
      theme_nothing()  
  } else {
  ...
  motif_sub <- splicing_mutation %>% 
    filter(Type_Motif == "donor", Rel_Start_Motif == pos) %>%
    filter(GSM2 == "Alternative 5' splice site")
  count_mat[pos, 3] <- nrow(motif_sub)
  
  if (count_mat[pos, 3] >= count_thres) {
    tgg_logo_A5S <- gg_seq_logo(t(get_base_count_mat(motif_sub$Motif_Seq, 9)), TRUE) + 
      labs(x = "", y = "") +
      theme_nothing()  
  } else {
  ...
  if (count_mat[pos, 4] >= count_thres) {
    tgg_logo_IR <- gg_seq_logo(t(get_base_count_mat(motif_sub$Motif_Seq, 9)), TRUE) + 
      labs(x = "", y = "") +
      theme_nothing()  
  } else {
  ...
  if (count_mat[pos, 5] >= count_thres) {
    tgg_logo_CP <- gg_seq_logo(t(get_base_count_mat(motif_sub$Motif_Seq, 9)), TRUE) + 
      labs(x = "", y = "") +
      theme_nothing()  
  } else {
...
ggsave("../output/seqlogo_list_detail.pdf", width = 11, height = 6)
