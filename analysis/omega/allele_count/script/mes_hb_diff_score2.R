...
snv_info <- read.table("../output/omega.splicing_mutation.info.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE, quote = "") %>%
...
g_mes_d <- ggplot(mes_df %>% filter(splice_class != "Alternative 3' splice site" & motif_type == "Donor"), aes(x = splice_class, y = mes_diff, fill = splice_class)) + 
...
g_mes_a <- ggplot(mes_df %>% filter(splice_class != "Alternative 5' splice site" & motif_type == "Acceptor"), aes(x = splice_class, y = mes_diff, fill = splice_class)) + 
  geom_boxplot(size = 0.3, outlier.size = 0.4) +
  coord_flip() +
...
ggsave("../output/diff_mes_spliceclass.pdf", width = 4, height = 4)
...
ggsave("../output/diff_mes_mutpos_donor.pdf", width = 6, height = 4)
...
ggsave("../output/diff_mes_mutpos_acceptor.pdf", width = 6, height = 4)
...
ggsave("../output/diff_hb_spliceclass.pdf", width = 6, height = 2.4)
...
ggsave("../output/diff_hb_mutpos_donor.pdf", width = 6, height = 4)
