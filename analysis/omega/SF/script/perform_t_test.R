...
# sf_gene <- commandArgs(trailingOnly=TRUE)[2]

# mut_list <- read.table(paste("../../output/", ctype, "/mut_SJ_IR_list.txt", sep = ""), sep ="\t", header = TRUE, stringsAsFactors = FALSE)
...
SF_mut <- read.table("../output/sample_list_with_sfm.txt", sep = "\t", stringsAsFactors = FALSE)
...
# asp_count <- read.table(paste("../../output/", ctype, "/", ctype, ".splicing.associate.txt", sep = ""), sep = "\t", header = TRUE,
...
for (i in 1:nrow(asp_count)) {
  ...
  Effect_Size[i] <- mean(sp_vec[sf_mut_id]) / (mean(sp_vec[setdiff(1:mut_num, sf_mut_id)]) + 1)
...
# sj_count <- read.table(paste("../../output/", ctype, "/", ctype, ".SJ_merged.annot.txt", sep = ""), sep = "\t", header = TRUE,
#                        stringsAsFactors = FALSE, quote = "") %>% 
...
ir_count <- read.table(paste("../../output/", ctype, "/", ctype, ".IR_merged.txt", sep = ""), sep = "\t", header = TRUE, stringsAsFactors = FALSE, quote = "")
