...
GC_info <- read.table("../output/omega.gsm.gc.txt", header = TRUE, sep = "\t", quote = "", stringsAsFactors = FALSE)
...
g_gc_d <- ggplot(GC_info_proc %>% 
  filter(Type_Motif == "Donor", Splice_Class2 != "Alternative 3' splice site", Is_Intron2 %in% c("5' intron", "Exon", "3' intron")), 
...
g_gc_a <- ggplot(GC_info_proc %>% 
  filter(Type_Motif == "Acceptor", Splice_Class2 != "Alternative 5' splice site", Is_Intron2 %in% c("5' intron", "Exon", "3' intron")),
  aes(x = Is_Intron2, y = GC_ratio, fill = Splice_Class2)) + 
  geom_boxplot(outlier.size = 0.6, size = 0.3) +
  labs(x = "", y = "GC contents") +
...
ggsave("../output/diff_gc_content.pdf", width = 8, height = 7)
...
g_dgc_d <- ggplot(GC_info_proc %>% 
  filter(Is_Intron == "GC_exon_intron_diff" & Type_Motif == "Donor", Splice_Class2 != "Alternative 3' splice site"),
...
g_dgc_a <- ggplot(GC_info_proc %>% 
  filter(Is_Intron == "GC_exon_intron_diff" & Type_Motif == "Acceptor", Splice_Class2 != "Alternative 5' splice site"),
  aes(x = Splice_Class2, y = GC_ratio, fill = Splice_Class2)) + 
  geom_boxplot(outlier.size = 0.6, size = 0.3) +
  labs(x = "", y = "Diff. of GC contents between exonic and intronic regions") +
  ggtitle("Acceptor disruption") +
...
ggsave("../output/diff_exon_intron_diff_gc_content.pdf", width = 6, height = 4)
...
ggsave("../output/diff_length.pdf", width = 8, height = 7)
