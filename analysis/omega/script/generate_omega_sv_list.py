...
mut_files = glob.glob(mut_dir + "/*.genomonSV.result.filt3.txt")
sj_files = glob.glob(sj_dir + "/*/*.SJ.out.tab")
ir_files = glob.glob(ir_dir + "/*.intron_retention.simple_count.txt")
...
for mut_file in sorted(mut_files):
    ...
    sample = os.path.basename(mut_file).replace(".genomonSV.result.filt3.txt", '')
    sample = sample[:15]
...
for sample in sorted(sample2mut_file):
    ...
    print >> hout, sample + '\t' + str(round(sample2weight[sample], 4)) + '\t' + \
                   sample2mut_file[sample] + '\t' + sample2sj_file[sample] + '\t' + \
