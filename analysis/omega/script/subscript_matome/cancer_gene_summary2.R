...
ggsave("../matome/cancer_gene_summary.pdf", width = 12, height = 12)
...
inframe_info <- splicing_mutation %>% 
  group_by(Sample_Name, Gene_Symbol, Mutation_Key) %>% 
  summarize(Is_Inframe_Paste = paste(Is_Inframe, collapse = ","),
            Supporting_Read_Num_Paste = paste(Supporting_Read_Num, collapse = ","))
...
is_inframe2 <- unlist(
  lapply(
    1:nrow(inframe_info),
    function(x) {
      treads <- as.numeric(strsplit(inframe_info$Supporting_Read_Num_Paste[x], ",")[[1]])
      tind <- which(treads == max(treads))[1]
      return(strsplit(inframe_info$Is_Inframe_Paste[x], ",")[[1]][tind])
    }
  )
  )

inframe_info$Is_Inframe2 <- ifelse(is_inframe2 == "in-frame", "Inframe", "Non_inframe")

inframe_info_proc <- inframe_info %>%
  group_by(Gene_Symbol, Is_Inframe2) %>% summarize(count = n()) %>%
  spread(key = Is_Inframe2, value = count, fill = 0) %>%
  mutate(Inframe_ratio = Inframe / (Inframe + Non_inframe)) %>%
  filter(Gene_Symbol %in% splicing_mutation_count_total$Gene_Symbol)


inframe_info_proc$Gene_Symbol2 <- factor(inframe_info_proc$Gene_Symbol,
                                    levels = splicing_mutation_count_total$Gene_Symbol)


p_inframe <- ggplot(inframe_info_proc, aes(x = Gene_Symbol2, y = Inframe_ratio)) + 
  geom_bar(stat = "identity") + 
  coord_flip() + theme_minimal() +
  labs(x = "", y = "") +
  theme(
