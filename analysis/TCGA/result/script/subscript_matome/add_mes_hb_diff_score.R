...
is_gsm <- factor(c(ifelse(snv_info_d$GSM2 != "no-change", "Abnormal splicing", "Normal splicing"), 
                   ifelse(snv_info_a$GSM2 != "no-change", "Abnormal splicing", "Normal splicing")),
                 levels = c("Abnormal splicing", "Normal splicing"))
...
mes_df <- data.frame(motif_type = factor(c(rep("donor", nrow(snv_info_d)), rep("acceptor", nrow(snv_info_a))),
                     ...
                     mes_wt = c(donor_mes_wt, acceptor_mes_wt),
...
write.table(mes_df, file = "../temporary/TCGA.savnet.allele_count.summary.mes.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

##########

is_gsm <- factor(ifelse(snv_info_d$GSM2 != "no-change", "Abnormal splicing", "Normal splicing"),
                 levels = c("Abnormal splicing", "Normal splicing"))

hb_df <- data.frame(hb_wt = hb_score_wt, hb_diff = c(hb_score_mt - hb_score_wt),
...
write.table(hb_df, file = "../temporary/TCGA.savnet.allele_count.summary.hb.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
plot_grid(plot_grid(g_mes_d, g_mes_a, ncol = 2, align = "h"), ylabel, ncol = 1, rel_heights = c(1, 0.15))
...
ylabel <- ggdraw() + draw_label("Wild type MaxEnt score", size = 7)
...
ggplot(hb_df %>% filter(splice_class != "Alternative 3'SS"), aes(x = splice_class, y = hb_wt, fill = splice_class)) +
  geom_boxplot(size = 0.3, outlier.size = 0.4) +
  coord_flip() +
  ylim(c(5, 25)) +
  my_theme() +
  scale_fill_manual(values = splicing_class_colour) +
  labs(x = "", y = "Wild type H-bond score") +
  guides(fill = FALSE)

ggsave("../figure/hb_wt_spliceclass.tiff", width = 6.5, height = 4.0, dpi = 600, units = "cm")
