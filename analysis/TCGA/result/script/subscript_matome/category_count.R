...
splicing_mutation$Mutation_Type3[
  splicing_mutation$Mutation_Type2 == "Donor disruption" &
    ...
    splicing_mutation$Is_Indel == FALSE] <- "SNV, non-canonical"
...
splicing_mutation$Mutation_Type3[
  splicing_mutation$Mutation_Type2 == "Donor disruption" &
    splicing_mutation$Is_Canonical2 == "non-canonical" &
    splicing_mutation$Is_Indel == TRUE] <- "Indel, non-canonical"
...
#     splicing_mutation$Mutation_Type3 %in% c("SNV, canonical donor disruption", "SNV, non-canonical donor disruption", 
#                                             "Indel, canonical donor disruption", "Indel, non-canonical donor disruption")] <- "Donor disruption"

# splicing_mutation$Mutation_Type4[
#     splicing_mutation$Mutation_Type3 %in% c("SNV, donor creation", "Indel, donor creation")] <- "Donor creation"

# splicing_mutation$Mutation_Type4[
#     splicing_mutation$Mutation_Type3 %in% c("SNV, canonical acceptor disruption", "SNV, non-canonical acceptor disruption",
#                                             "Indel, canonical acceptor disruption", "Indel, non-canonical acceptor disruption")] <- "Acceptor disruption"
...
mut_type3_order <- rev(c("SNV, canonical", "SNV, non-canonical", "SNV", "Indel, canonical", "Indel, non-canonical", "Indel"))
...
splicing_mutation_type_count2 <- splicing_mutation %>%
    ...
    group_by(Mutation_Type3, Splicing_Class) %>%
    summarize(count = n()) %>%
    mutate(countf = prettyNum(count, big.mark = ",", scientific = FALSE))
...
splicing_mutation_type_count2$Mutation_Type32 <- factor(splicing_mutation_type_count2$Mutation_Type3,
    ...
    labels = unlist(lapply(mut_type3_order, 
                           function(x) {paste(x, " (n = ", splicing_mutation_type_count3$countf[splicing_mutation_type_count3$Mutation_Type3 == x], ")", sep = "")})))
...
p_dd <-ggplot(splicing_mutation_type_count2, aes(Mutation_Type32, y = count, fill = Splicing_Class)) + 
  ...
  coord_flip() +
  ggtitle(paste("Donor disruption (n = ", format(sum(splicing_mutation_type_count3$count), big.mark = ",", scientific = FALSE), ")", sep ="")) +
  labs(x = "", y = "", fill = "") +
  my_theme() +
  theme(legend.position = "bottom") +
  scale_fill_manual(breaks = c("Exon skipping", "Alternative 5'SS", "Alternative 3'SS", "Intron retention"),
                    values = splicing_class_colour) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 5300), labels = scales::comma_format()) +
  guides(fill = FALSE)
...
splicing_mutation_type_count2$Mutation_Type32 <- factor(splicing_mutation_type_count2$Mutation_Type3,
    levels = mut_type3_order,
    labels = unlist(lapply(mut_type3_order,
                           function(x) {paste(x, " (n = ", as.character(splicing_mutation_type_count3$countf[splicing_mutation_type_count3$Mutation_Type3 == x]), ")", sep = "")})))

print(splicing_mutation_type_count2$Mutation_Type32)


p_dc <- ggplot(splicing_mutation_type_count2, aes(Mutation_Type32, y = count, fill = Splicing_Class)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  ggtitle(paste("Donor creation (n = ", format(sum(splicing_mutation_type_count3$count), big.mark = ",", scientific = FALSE), ")", sep ="")) +
...
p_ad <- ggplot(splicing_mutation_type_count2, aes(Mutation_Type32, y = count, fill = Splicing_Class)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  ggtitle(paste("Acceptor disruption (n = ", format(sum(splicing_mutation_type_count3$count), big.mark = ",", scientific = FALSE), ")", sep ="")) +
  labs(x = "", y = "", fill = "") +
  my_theme() +
  theme(legend.position = "bottom") +
  scale_fill_manual(breaks = c("Exon skipping", "Alternative 5'SS", "Alternative 3'SS", "Intron retention"),
...
p_ac <- ggplot(splicing_mutation_type_count2, aes(Mutation_Type32, y = count, fill = Splicing_Class)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  ggtitle(paste("Acceptor creation (n = ", format(sum(splicing_mutation_type_count3$count), big.mark = ",", scientific = FALSE), ")", sep ="")) +
  labs(x = "", y = "Splicing event count", fill = "") +
  my_theme() +
  theme(legend.position = "bottom") +
  scale_fill_manual(breaks = c("Exon skipping", "Alternative 5'SS", "Alternative 3'SS", "Intron retention"),
...
p_dd_ad_dc_ac <- plot_grid(p_dd, p_ad, p_dc, p_ac, ncol = 2, align = "hv", rel_heights = c(1, 0.72))

plot_grid(p_dd_ad_dc_ac, g_legend(p_dummy_for_legend), ncol = 1, rel_heights = c(1, 0.1))



ggsave("../figure/category_count.tiff", width = 18, height = 8.5, dpi = 600, units = "cm")
